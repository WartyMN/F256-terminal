...
void App_TerminalMainLoop(void);
	...
	while (! exit_main_loop)
	{
		...
		do
		{
			...
			switch (user_input)
			{
				...
				case ACTION_DIAL_BBS:
					App_TerminalMainLoop();
					break;
...
// 	uint8_t				back_color = COLOR_BLACK;
	...
	while (! exit_main_loop)
	{
		...
		Text_FillBox(UI_LEFT_PANEL_BODY_X1, UI_VIEW_PANEL_BODY_Y1, UI_LEFT_PANEL_BODY_X2, UI_VIEW_PANEL_BODY_Y2, CH_SPACE, COLOR_ORANGE, COLOR_BLACK);

		// prep for re-outputting serial to top part of serial panel area
		zp_x = UI_LEFT_PANEL_BODY_X1;
		zp_y = UI_VIEW_PANEL_BODY_Y1;
...
// 				else
// 				{
// 					Text_SetCharAndColorAtXY(x++, y, in_byte, fore_color, back_color);
// 					
// 					if (x > 79)
// 					{
// 						x = 0;
...
// 			asm("stz $01");
// 			(__A__ = y, asm("sta $d016"));
// 			(__A__ = x, asm("sta $d014"));
...
void App_DisplayTime(void)
{
	...
	Text_DrawStringAtXY(64, 50, global_string_buff1, COLOR_BRIGHT_YELLOW, COLOR_BLACK);
}
